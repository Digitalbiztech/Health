    ...
    <div className="glass-card rounded-2xl p-4 border-border/40 shadow-sm flex flex-col gap-3 max-h-[calc(100vh-100px)]">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-bold text-foreground flex items-center gap-2">
          <ListTodo className="w-4 h-4 text-[var(--primary-text)]" />
          Tasks
        </h4>
        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--primary)]/15 text-[var(--primary-text)]">
          {openCount} open
        </span>
      </div>
      ...
      <form onSubmit={submit} className="flex flex-col gap-2">
        <input
          ...
          className="w-full text-xs rounded-xl px-3 py-2 outline-none border border-border/60 bg-card text-foreground focus:border-[var(--primary)] transition-colors"
        />
        ...
        {(['open', 'done'] as const).map((f) => (
          <button
            ...
            className={cn(
              ...
              filter === f ? 'bg-[var(--primary)]/15 text-[var(--primary-text)]' : 'text-muted-foreground hover:text-foreground',
            )}
          >
            {f}
          </button>
        ...
        {loading ? (
          ...
          visible.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className="group p-2.5 rounded-xl border border-border/40 bg-card/65 flex items-start gap-2.5 hover:border-[var(--primary)]/50 transition-colors"
              >
                <button
                ...
                >
                  {done ? (
                    ...
                    <Circle className="w-4 h-4 text-muted-foreground hover:text-[var(--primary-text)]" />
                  )}
                </button>
